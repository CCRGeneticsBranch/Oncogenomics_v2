...
class Project extends Model {
	...
	public function GenotypingByPatient($patient_id) {
		$sql="select s1.patient_id as Patient1,g.sample1, c1.cnt as sample1_count, s1.tissue_type as Tissue1,s1.exp_type as exp_type1, s2.patient_id as Patient2,g.sample2, c2.cnt as sample2_count, s2.tissue_type as Tissue2,s2.exp_type as exp_type2,percent_match from genotyping g left join genotyping_count c1 on g.sample1=c1.sample_id or g.sample1=c1.sample_name left join genotyping_count c2 on g.sample2=c2.sample_id or g.sample2=c2.sample_name,project_samples s1,project_samples s2 where (g.sample1=s1.sample_id or g.sample1=s1.sample_name) and (g.sample2=s2.sample_id or g.sample2=s2.sample_name) and s1.project_id=$this->id and s2.project_id=$this->id and (s1.patient_id='$patient_id' or s2.patient_id='$patient_id')";
		return DB::select($sql);
